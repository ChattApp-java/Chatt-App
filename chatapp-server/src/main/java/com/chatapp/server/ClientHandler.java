    ...
    private void handleMessage(String raw) {
        ...
        switch (type) {
            ...
            case Protocol.MSG_ACK: {
                ...
                String dest = parts[1];
                ClientHandler target = server.getClient(dest);
                if (target != null) target.sendMessage(Protocol.MSG_ACK + Protocol.SEP + username);
                break;
            }
            ...
            case Protocol.MSG_READ: {
                ...
                String dest = parts[1];
                ClientHandler target = server.getClient(dest);
                if (target != null) target.sendMessage(Protocol.MSG_READ + Protocol.SEP + username);
                break;
            }
            ...
            case Protocol.TYPING_START: {
                ...
                String dest = parts[1];
                ClientHandler target = server.getClient(dest);
                if (target != null) target.sendMessage(Protocol.TYPING_START + Protocol.SEP + username);
                break;
            }
            ...
            case Protocol.TYPING_STOP: {
                ...
                String dest = parts[1];
                ClientHandler target = server.getClient(dest);
                if (target != null) target.sendMessage(Protocol.TYPING_STOP + Protocol.SEP + username);
                break;
            }
            ...
            // ──────────────────────────────────────────────────
            // FILE|destinataire|nomFichier|base64Content
            // ──────────────────────────────────────────────────
            case Protocol.FILE: {
                if (parts.length < 4) break;
                String dest    = parts[1];
                ...
                ClientHandler target = server.getClient(dest);
                if (target != null) {
                    target.sendMessage(Protocol.FILE_RECV + Protocol.SEP
                            + username + Protocol.SEP + fileName + Protocol.SEP + b64);
            ...
            case Protocol.AUDIO_MSG: {
                ...
                ClientHandler target = server.getClient(dest);
                if (target != null) {
                    target.sendMessage(Protocol.AUDIO_RECV + Protocol.SEP
                            + username + Protocol.SEP + b64);
